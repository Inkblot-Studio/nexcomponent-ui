 ...
 */
const NexHeroCard = ({ title, subtitle, type, buttonLabel, buttonHandle, backgroundUrl, className }) => {
    ...
    const backgroundStyle = backgroundUrl ? { '--background-url': `url(${backgroundUrl})` } : {};
    return (<div className={heroCardClasses} style={backgroundStyle}>
      {!backgroundUrl && (<div className="background-blob"></div>)}

      <div className="content">
        <div className="title">{title}</div>
        <div className="subtitle">{subtitle}</div>
        {buttonLabel && buttonHandle && (<NexButton type={backgroundUrl ? 'glass' : ''} size='large' text={buttonLabel} onClick={buttonHandle} className='hero-button'></NexButton>)}
      </div>
    </div>);
};
export default NexHeroCard;
//# sourceMappingURL=NexHeroCard.jsx.map
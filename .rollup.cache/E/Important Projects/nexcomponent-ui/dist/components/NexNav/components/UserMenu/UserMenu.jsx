...
const UserMenu = ({ user, onLogout, onProfile, onEndorsementsClick, onSubscriptionClick, onActivityLogClick, onSecurityClick, onIntegrationsClick, onAdminPanelClick, onSignUpClick, endorsementCount, subscription, enableEndorsements, enableSubscriptionInfo, enableAuditLog, enableSecuritySettings, enableIntegrations, enableAdminPanel, isAtTop = true, open = false, onOpen, onClose, theme = 'auto' }) => {
    ...
    return (<div className="nex-user-menu" ref={ref}>
      <motion.button className="nex-user-menu-trigger" onClick={handleToggle} aria-haspopup="true" aria-expanded={open} aria-label="User menu">
        {user?.avatarUrl ? (<img src={user.avatarUrl} alt="User Avatar" className="nex-user-menu-avatar"/>) : (<span className="nex-user-menu-avatar nex-user-menu-avatar-fallback">
      ...
      <AnimatePresence>
        {open && (<motion.ul className="nex-user-menu-dropdown" initial={{ opacity: 0, y: -8 }} animate={{
                opacity: 1,
                y: 0,
            ...
            }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2, ease: [0.4, 0, 0.2, 1] }} role="menu">
            {user ? (<>
                ...
                {onProfile && (<motion.li className="nex-user-menu-item" role="menuitem" onClick={() => {
                        onClose && onClose();
                        onProfile();
                    }} whileHover={hoverAnimation} whileTap={tapAnimation} style={{ cursor: 'pointer' }}>
                    Profile
                  </motion.li>)}

                {enableEndorsements && (<motion.li className="nex-user-menu-item" role="menuitem" onClick={() => {
                        onClose && onClose();
                        onEndorsementsClick?.();
                    }} whileHover={hoverAnimation} whileTap={tapAnimation} style={{ cursor: 'pointer' }}>
                ...
                {enableSubscriptionInfo && subscription && (<motion.li className="nex-user-menu-item" role="menuitem" onClick={() => {
                        onClose && onClose();
                        onSubscriptionClick?.();
                    }} whileHover={hoverAnimation} whileTap={tapAnimation} style={{ cursor: 'pointer' }}>
                ...
                {enableAuditLog && (<motion.li className="nex-user-menu-item" onClick={() => {
                        onClose && onClose();
                        onActivityLogClick?.();
                    }} whileHover={hoverAnimation} whileTap={tapAnimation} style={{ cursor: 'pointer' }}>
                    Activity Log
                ...
                {enableSecuritySettings && (<motion.li className="nex-user-menu-item" onClick={() => {
                        onClose && onClose();
                        onSecurityClick?.();
                    }} whileHover={hoverAnimation} whileTap={tapAnimation} style={{ cursor: 'pointer' }}>
                    Security Settings
                ...
                {enableIntegrations && (<motion.li className="nex-user-menu-item" onClick={() => {
                        onClose && onClose();
                        onIntegrationsClick?.();
                    }} whileHover={hoverAnimation} whileTap={tapAnimation} style={{ cursor: 'pointer' }}>
                    Manage Integrations
                ...
                {enableAdminPanel && (<motion.li className="nex-user-menu-item" onClick={() => {
                        onClose && onClose();
                        onAdminPanelClick?.();
                    }} whileHover={hoverAnimation} whileTap={tapAnimation} style={{ cursor: 'pointer' }}>
                    Admin Panel
                ...
                {onLogout && (<motion.li className="nex-user-menu-item danger" onClick={() => {
                        onClose && onClose();
                        onLogout();
                    }} whileHover={dangerHoverAnimation} whileTap={dangerTapAnimation} style={{ cursor: 'pointer' }}>
                    Log out
                  </motion.li>)}
              </>) : (<motion.li className="nex-user-menu-item sign-up-cta" onClick={() => {
                    onClose && onClose();
                    onSignUpClick?.();
                }} whileHover={hoverAnimation} whileTap={tapAnimation} style={{ cursor: 'pointer' }}>

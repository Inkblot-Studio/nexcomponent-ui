...
const FooterBottom = ({ displayName, socials = [], variant = 'default', theme = 'auto' }) => {
    ...
    return (<motion.div className={bottomClass} variants={sectionVariants}>
      <div className="nex-footer-bottom__container">
        <motion.div className="nex-footer-bottom__copyright" whileHover={{
            opacity: 0.9
        ...
        {/* Social Links */}
        {socials.length > 0 && (<motion.div className="nex-footer-bottom__socials" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, ...timing.medium }}>
            {socials.map((social, index) => {
                const IconComponent = socialIcons[social.type];
                if (!IconComponent)
                    return null;
                return (<motion.a key={index} href={social.url} target="_blank" rel="noopener noreferrer" className={`nex-footer-bottom__social-link ${social.type}`} aria-label={`Follow us on ${social.type}`} whileHover={{
                        opacity: 0.9,
...
//# sourceMappingURL=FooterBottom.jsx.map
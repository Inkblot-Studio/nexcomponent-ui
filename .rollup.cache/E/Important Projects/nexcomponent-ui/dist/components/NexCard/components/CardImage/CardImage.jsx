...
const CardImage = ({ src, alt, className }) => {
    ...
    if (!src)
        return null;
    return (<motion.div className={`nex-card-image ${className || ''}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{
            duration: shouldReduceMotion ? 0.2 : 0.5,
            ease: [0.4, 0, 0.2, 1]
        }}>
      <img src={src} alt={alt || 'Card image'} className="nex-card-image-element"/>
    </motion.div>);
};
export default CardImage;
//# sourceMappingURL=CardImage.jsx.map
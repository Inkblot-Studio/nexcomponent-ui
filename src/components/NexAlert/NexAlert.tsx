 ...
 * ```
 */
const NexAlert: React.FC<NexAlertProps> = ({
  id,
  type = 'info',
  variant = 'default',
  size = 'md',
  ...
  const totalDurationRef = useRef<number>(timeout);
  ...
  const alertIcon = icon || defaultIcons[type as keyof typeof defaultIcons];
    ...
    <AnimatePresence>
      {isVisible && (
        ...
        >
          ...
          <div className={styles.content}>
            ...
            {(title || message) && (
              <div className={styles.header}>
                {title && (
                  <h4 className={`${styles.title} ${styles[`size${size.charAt(0).toUpperCase() + size.slice(1)}`]}`}>
                    {title}
                  </h4>
                )}
                {message && !title && (
                  <p className={`${styles.message} ${styles[`size${size.charAt(0).toUpperCase() + size.slice(1)}`]}`}>
                    {message}
                  </p>
                )}
              </div>
            ...
            {actions.length > 0 && (
              <div className={styles.actions}>
                {actions.map((action, index) => (
                  <button
                    key={index}
                    className={`${styles.actionButton} ${action.variant === 'primary' ? styles.primary : ''}`}
          ...
          {/* Progress bar */}
          {timeout > 0 && !persistent && !shouldReduceMotion && (
            <div 
